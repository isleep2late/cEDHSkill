...
- **Changed: confirmations and turn order now use buttons instead of reactions.** Game messages carry a ✅ Confirm / Cancel row and Turn 1–4 buttons. Only players in the game can confirm; an admin/moderator can supply the final missing confirmation; each player holds at most one turn (click again to rescind, claim a taken turn to take it over). Turn buttons keep working after the game is confirmed — even across bot restarts — for up to 1 hour after submission, then the window closes. The `GuildMessageReactions` intent and the optional Manage Messages permission are no longer needed.
...
**Turn order:** Can be specified inline with a number after the result (`@user w 1` = Turn 1), or with the **Turn 1–4 buttons** on the game message. Each player can hold only one turn; clicking your current turn again rescinds it, and claiming a turn someone else holds takes it over. The turn buttons keep working after the game is confirmed, for up to **1 hour after submission** — then the window closes and the buttons disappear.

**Game confirmation:** After submitting, all mentioned players must click the ✅ **Confirm** button before ratings are applied (the submitter can cancel with the red Cancel button). If only one confirmation is missing, an admin or moderator can click ✅ Confirm to push the game through. Pending games expire after 1 hour.
