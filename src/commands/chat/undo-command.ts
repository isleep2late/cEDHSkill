...
export class UndoCommand implements Command {
    ...
    public async execute(intr: ChatInputCommandInteraction, data: EventData): Promise<void> {
        ...
        if (
            ...
            RankCommand.latestConfirmedRankOpDetails.guildId === guildId
        ) {
            ...
            undoEmbed.setDescription(
                Lang.getRef('undoMessages.confirmedDescriptionText', data.lang)
            );
            ...
            try {
                for (const player of playersToRevert) {
                    ...
                    const currentElo = RatingUtils.calculateElo(
                        player.newRating.mu,
                    ...
                    try {
                        ...
                        if (channel?.isTextBased()) {
                            const originalMessage = await channel.messages.fetch(
                                confirmedOp.messageId
                            );
                            if (originalMessage && originalMessage.embeds[0]) {
                                const undidEmbed = new EmbedBuilder({
                                    ...originalMessage.embeds[0].data,
                                }); // Copy existing
                                undidEmbed.setTitle(
                                    Lang.getRef('fields.rankUndoneTitle', data.lang)
                                );
                                undidEmbed.setDescription(
                                    Lang.getRef('undoMessages.rankUndoneText', data.lang)
                                );
                        ...
                        console.warn(
                            `UndoCommand: Could not edit original confirmed rank message ${confirmedOp.messageId}:`,
                            error
                        );
                ...
                await InteractionUtils.send(
                    intr,
                    Lang.getEmbed('errorEmbeds.undoFailed', data.lang),
                    true
                );
            }
            return;
        ...
        if (
            ...
            RankCommand.latestPendingRankContext.guildId === guildId
        ) {
            ...
            if (pendingUpdate && pendingUpdate.status !== 'disabled_by_undo') {
                ...
                try {
                    ...
                    if (
                        ...
                        await InteractionUtils.send(
                            intr,
                            Lang.getEmbed('displayEmbeds.undoPendingSuccess', data.lang)
                        );
                ...
                await InteractionUtils.send(
                    intr,
                    Lang.getEmbed('displayEmbeds.undoAlreadyDisabled', data.lang),
                    true
                );
            } else {
                ...
                await InteractionUtils.send(
                    intr,
                    Lang.getEmbed('displayEmbeds.undoNothingToUndo', data.lang),
                    true
                );
            }
            return;
