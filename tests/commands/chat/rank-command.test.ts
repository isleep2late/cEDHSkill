...
describe('RankCommand', () => {
    ...
    beforeEach(async () => {
        ...
        mockClientUsersFetch = vi.fn(async (id: string): Promise<User | null> => {
            if (id === '123') return { id: '123', username: 'User123', tag: 'User123#0001' } as User;
            if (id === '456') return { id: '456', username: 'User456', tag: 'User456#0002' } as User;
            if (id === '789') return { id: '789', username: 'User789', tag: 'User789#0003' } as User;
            if (id === '101') return { id: '101', username: 'User101', tag: 'User101#0004' } as User;
            return null;
        });
    ...
    it('should send "invalid outcome" if only winners are provided', async () => {
        ...
        mockPlayerRatingFindOneFn.mockResolvedValue({ userId: 'any', guildId: MOCK_GUILD_ID, mu: 25, sigma: 8, wins: 0, losses: 0 } as any);
        await rankCommand.execute(mockIntr, mockEventData);
        expect(interactionUtilsSendMock).toHaveBeenCalledWith(mockIntr, currentMockEmbed, true);
